...
def train_linear_probe(defaults_path="configs/defaults.yaml",
    ...
    if os.path.exists(pretrained_weights):
        ...
        # Since output activation is different but shape is the same, load_weights still works
        model.load_weights(pretrained_weights)
    else:
        print(f"WARNING: Pre-trained weights not found at {pretrained_weights}! Training from scratch.")

    # FREEZE FEATURE EXTRACTOR
    print("Freezing all layers except the final classification head...")
