...
drive model classification decisions using raw pre-softmax class scores z_k.
"""
...
class GradCamResult:
    ...
    def __iter__(self):
        # Enables tuple unpacking for backwards compatibility:
        # display_cam, predicted_class, confidence = compute_gradcam(...)
...
def _extract_logits_and_conv(
    ...
    Raises:
        ValueError: if pre-softmax score z_k cannot be extracted.
    """
    ...
    if is_softmax:
        if isinstance(last_layer, tf.keras.layers.Dense):
            # Dense layer with softmax activation: z = hW + b
            ...
            # Softmax layer following logits layer
            submodel = tf.keras.Model(
                inputs=model.inputs,
                outputs=[conv_layer.output, last_layer.input],
            )
...
def compute_gradcam(
    ...
    Target score is the true pre-softmax class score z_k.
    Maintains rank-2 batch dimension [B, T] for all outputs.

    Args:
        model: Trained Keras model.
        inputs: Input audio tensor/array of shape [B, T, C] or [B, T] or [T].
        class_idx: Target class index. If None, uses top predicted class per sample.
        layer_name: Target conv layer name. If None, auto-detects last conv layer.

    Returns:
        GradCamResult containing raw_cam, display_cam, stats, class_idx, confidence.
        Supports 3-tuple unpacking: (display_cam, class_idx, confidence).
    """
    if tf is None:
    ...
    # Channel-wise average pooling across spatial/temporal dimension [B, T', C]
    pooled_grads = tf.reduce_mean(grads, axis=1)  # shape [B, C]

    # Weighted combination of feature maps
    raw_cam = tf.reduce_sum(
        conv_outputs * pooled_grads[:, tf.newaxis, :], axis=-1
    )  # shape [B, T']
    raw_cam = tf.maximum(raw_cam, 0.0)  # shape [B, T']

    conv_T = tf.shape(conv_outputs)[1]

    # Temporal Alignment to input T when T' differs from T (Bilinear interpolation approximation)
    if conv_T != target_T:
    ...
    raw_cam_np = raw_cam_aligned.numpy()  # [B, T]
...
def aggregate_raw_cams(
    ...
    """Aggregate raw analytical CAMs across multiple samples preserving magnitude differences.

    Args:
        raw_cams: Array or sequence of raw CAM vectors of shape [N, T] or list of [T] arrays.

    Returns:
        Dict containing count, mean, variance, std, median, and quantiles (0.25, 0.75).
    """
